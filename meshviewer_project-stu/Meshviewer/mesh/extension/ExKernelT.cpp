...
namespace MeshN { 
	...
	void ExKernelT<ExItems>::Laplacian_Smoothing() {
		...
		while (iterations--)
		{
			...
			int cur = 0;
			for (VertexIterator vit = vertex_begin(); vit != vertex_end(); vit++)
			{
				HalfedgeHandle& first = vit->halfedge_handle_;
				HalfedgeHandle& other = opposite_halfedge_handle(first);
				int cnt = 0;
				do
				{
					updateVertexPosition[cur] = updateVertexPosition[cur] + coord(vertex_handle(other));
					cnt = cnt + 1;
					other = prev_halfedge_handle(other);
					other = opposite_halfedge_handle(other);
				} while (opposite_halfedge_handle(other) != first);
				updateVertexPosition[cur] /= cnt;
				cur++;
			}
			cur = 0;
			for (VertexIterator vit = vertex_begin(); vit != vertex_end(); vit++)
			{
				vit->coord_ = updateVertexPosition[cur];
				cur++;
			}
		}
		end = clock();
		std::cout << "The time of Laplacian_Smoothing vertex updating: " << (end - start) * 1.0 / CLOCKS_PER_SEC << "s" << std::endl;
	}
...
////////////////////////////////////////////////////////////////////////////////////
	...
	void ExKernelT<ExItems>::mesh_process(){///////(可选，在三角网格上实现一种操作，例如特征提取，网格分割，三角网格变形等等)
		//-----------------------------------
		// 网格细分代码见read_write.cpp中ogl_writer2下
		// 将网格细分代码替换ogl_writer代码即可
		// 按m即可查看网格细分效果
		//-----------------------------------
	}
	...
	typename ExKernelT<ExItems>::Scalar
		...
		double e1, e2, e3;
		///得到半边句柄
		HalfedgeHandle first_halfedge = halfedge_handle(_fh);
		HalfedgeHandle second_halfedge = next_halfedge_handle(first_halfedge);
		HalfedgeHandle third_halfedge = next_halfedge_handle(second_halfedge);
		////由半边句柄得到边句柄
		EdgeHandle first_edge = edge_handle(first_halfedge);
		EdgeHandle second_edge = edge_handle(second_halfedge);
		EdgeHandle third_edge = edge_handle(third_halfedge);
		////由边句柄得到各边长
		e1 = calc_edge_length(first_edge);
		e2 = calc_edge_length(second_edge);
		e3 = calc_edge_length(third_edge);
		////利用海伦公式求面积s=sqrt(p(p-a)(p-b)(p-c))
		Scalar area = 0.0;
		double p = 0.5 * (e1 + e2 + e3);
		area = (float)sqrt(p * (p - e1) * (p - e2) * (p - e3));
...
//////////////////////////////////////////////////////////////////////////////////
	...
	typename ExKernelT<ExItems>::Normal
		...
		//////////////////////////在此实现////////////////////////////////////
		HalfedgeHandle first, other;
		FacetHandle temp_face;

		first = halfedge_handle(_vh);
		other = next_halfedge_handle(first);
		other = opposite_halfedge_handle(other);

		double FacetAreaSum = 0;

		while (first != other)   //半边数据结构不同于链式前向星，只能通过这种方式遍历所有与它相连的边和其周围的三角面片
		{
			temp_face = facet_handle(other);
			FacetAreaSum = FacetAreaSum + calc_facet_area(temp_face);
			other = next_halfedge_handle(other);
			other = opposite_halfedge_handle(other);
		}  //给与该顶点所有相连的面片求个面积的总和，以便下一步以权重求和

		temp_face = facet_handle(first);
		other = next_halfedge_handle(first);
		other = opposite_halfedge_handle(other);
		...
		{
			temp_face = facet_handle(other);
			norm = norm + calc_normal(temp_face) * calc_facet_area(temp_face) / FacetAreaSum; //依照面积大小作为权重对周围的法向求和
			other = next_halfedge_handle(other);
			other = opposite_halfedge_handle(other);
		}
		return norm;
